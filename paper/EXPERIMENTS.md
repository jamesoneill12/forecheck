...
| v5: 30 policy kinds, 4 withheld, decoder 2B | `configs/eks/train-2b-b200-v5-recipe.yaml` (ADR 0011) | done: macro 0.957/0.951/0.941/0.970; `heldout_policy_kind` `policy_conflict` 0.704 -- within seed noise of v4, does not fix kind generalisation; seed 1 running (`2b-v5-seed1`) |
| Agent self-judgment, decoder-comparable split coverage (test + adversarial for both 2B/8B, full identity + stripped) | `configs/eks/eval-agent-self-b200-recipe.yaml` | done for heldout_family (2B, 8B) and adversarial (8B); 8B-v4 checker comparison running (`eval-v4-idself`); test split and 2B/adversarial not yet run |
...
| Decoder 2B v4, identity-stripped (fills Table 1's `---` cell) | `configs/eks/train-2b-b200-v4-recipe.yaml` eval with `--strip-identity` | running |
