...
\citep{agentactionguard2025} is an embedding MLP over tool name and
arguments. LlamaFirewall's AlignmentCheck
\citep{chennabasappa2025llamafirewall} and Task Shield
\citep{jia2025taskshield} judge whether an action is aligned with the user's
stated goal, the closest deployed analogues of our
\texttt{unauthorized\_scope}; neither takes a typed entitlement or delegated
scope as input.
...
\texttt{unauthorized\_scope}. Recent agent-specific designs enforce authorization by
...
programmable per-call privilege policies, IsolateGPT \citep{wu2025isolategpt}
isolates agent applications behind a permission layer, RTBAS
\citep{zhong2025rtbas} and contextual-integrity work
\citep{bagdasarian2024airgapagent} enforce information-flow norms that depend
on who the principal is. OWASP's agentic guidance \citep{owasp2025agentic}
holds that authorization must be deterministic and auditable. Our delta is
...
across paraphrases (\S\ref{sec:generalisation}) and emits a calibrated
probability an engine of this kind can consume, while the deterministic
check on typed fields remains the right tool for scope.
