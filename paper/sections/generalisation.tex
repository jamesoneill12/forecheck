...
data, \texttt{policy\_conflict} only. Source:
...
while withholding the same proportion (4 of 30 vs.\ 2 of 15). Result:
\texttt{policy\_conflict} on \texttt{heldout\_policy\_kind} lands at 0.704
AUPRC (AUROC 0.773) -- essentially unchanged from v4's 0.752, and inside the
0.689--0.752 range the same 2B recipe already shows across two training
seeds (\S\ref{sec:limitations}). More kinds does not fix kind-generalization
at 2B. The rule baseline on this split is 0.325, so the model is still far
above chance; it has just not learned the general operation.
\textbf{Model size helps, partially.} An 8B LoRA on the unchanged v4 data
reaches 0.851 AUPRC (AUROC 0.916) on the same \texttt{heldout\_policy\_kind}
\texttt{policy\_conflict} cell -- a real gain over 2B's 0.752 -- but still
well short of the $\sim$0.98 phrasing-generalization ceiling both sizes
reach. Capacity recovers some of the missing generalization; it does not
close the gap.
